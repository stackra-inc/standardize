...
import { readdirSync, statSync, existsSync, readFileSync, writeFileSync, mkdirSync, chmodSync } from 'fs';
...
function ensureFile(filePath, content, results, pkgDir, checkName, auditOnly, executable = false, createOnly = false) {
  ...
  if (!existsSync(filePath)) {
    ...
    if (!auditOnly) {
      ...
      if (executable) chmodSync(filePath, 493);
    }
    return false;
  }
  if (createOnly) {
...
\u{1F4E6} ${name} (${dirName}) \u2014 ${pkgType}`);
  ...
  const steeringSource = resolve(__dirname$1, "..", "..", "steering");
  if (existsSync(steeringSource)) {
    const steeringFiles = readdirSync(steeringSource).filter(
      (f) => f.endsWith(".md")
    );
  ...
  if (!auditOnly && failed > 0) {
    ...
    for (const dir of packageDirs) {
      ...
      if (pkg.scripts?.format) {
        try {
          execSync("pnpm format", { cwd: dir, stdio: "pipe", timeout: 3e4 });
        } catch {
        }
      }
    }
